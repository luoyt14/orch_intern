...
from data.constant import tusimple_row_anchor, culane_row_anchor, water_row_anchor
...
def get_train_loader(batch_size, data_root, griding_num, dataset, use_aux, distributed, num_lanes):
    target_transform = transforms.Compose([
        mytransforms.FreeScaleMask((640, 320)),
        mytransforms.MaskToTensor(),
    ])
    segment_transform = transforms.Compose([
        mytransforms.FreeScaleMask((80, 40)),
        mytransforms.MaskToTensor(),
    ])
    ...
    if dataset == 'CULane':
        ...
        train_dataset = LaneClsDataset(data_root,
                                           os.path.join(data_root, 'list/train_gt_onlyori.txt'), isrotate=True,
                                           img_transform=img_transform, target_transform=target_transform,
                                           simu_transform = simu_transform,
                                           griding_num=griding_num, 
...
def get_test_loader(batch_size, data_root,dataset, distributed):
    ...
    if dataset == 'CULane':
        ...
        test_dataset = LaneTestDataset(data_root, os.path.join(data_root, "list/test.txt"), isrotate=True, img_transform=img_transforms)
        cls_num_per_lane = 64
