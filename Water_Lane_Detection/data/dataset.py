...
class LaneTestDataset(torch.utils.data.Dataset):
    def __init__(self, path, list_path, isrotate = False, img_transform=None):
...
class LaneClsDataset(torch.utils.data.Dataset):
    def __init__(self, path, list_path, isrotate = False, img_transform = None,target_transform = None,simu_transform = None, griding_num=50, load_name = False,
                row_anchor = None,use_aux=False,segment_transform=None, num_lanes = 1):
    ...
    def _get_index(self, label):
        ...
        if h != 640:
            scale_f = lambda x : int((x * 1.0/640) * h)
